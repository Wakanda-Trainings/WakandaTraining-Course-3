function myLogin(userName, password) //simple login listener
{
	...
	else
	{
		...
		if (userID != 0)
			return {ID: userID, //UUID (converted if necessary)
					name: userName, //user login
					fullName: userName, //friendly display name
					belongsTo: theGroups}; //array of groups
		else
			return {error: 1024, errorMessage: 'Invalid Login'};
	}
};

function myLogin2(userName, password) //advanced login listener
{
...
function myLogin3(userName, password, passwordIsKey) //uses a key instead of a plain text password
{
	var theUser = directory.internalStore.User({name: userName});
	if (theUser != null) //user exists in the directory
		return false; // allow directory authentication
	...
	{      //default will be to reject login
		...
		if (theEmployee != null)// if a user was found
		{
			...
			var hashValue = '';
			if (passwordIsKey)
				hashValue = Sha256.hash(password + 'Wakanda');
			else
				hashValue = directory.computeHA1(userName, password)
			if (theEmployee.password === hashValue) 
			{
				var theGroups = [];
				var putIntoStorage = {myID: theEmployee.ID};
				switch (theEmployee.accessLevel){
...
function organizationLogin(userName, password) //Organization login listener
{
	var theUser = directory.internalStore.User({name: userName});
	if (theUser != null) //user exists in the directory
		return false; // allow directory authentication
	...
	{      //default will be to reject login
		var result = {error: 1024, errorMessage: 'Invalid Login'};
		var organizationDs = solution.getApplicationByName("Organization").ds;
		var theEmployee = organizationDs.Employee({login:userName});
		if (theEmployee != null) // if a user was found
		{
			// see of the stored hash value is correct
			if (theEmployee.password === directory.computeHA1(userName, password)) 
			{
				...
				var putIntoStorage = {myEmployeeID: theEmployee.ID};
				if (theEmployee.manager != null)
					putIntoStorage.myManagerID = theEmployee.manager.ID;
				var theGroups = ['Worker'];
				...
				result = {
					...
					belongsTo: theGroups,
					storage : {loginInfo: putIntoStorage}
				};
			}
		}
		return result; // either an error or a custom user object
